    ...
    def __reduce_matrix__(self, coordinates, min_value):
        ...
        if all([x < (float(self.distance_matrix.shape[0]) / 2.) for x in list(coordinates)]):
            for i in range(self.distance_matrix.shape[0]):
                column.append(self.distance_func(
                    (self.distance_matrix[i][coordinates[0]], self.distance_matrix[i][coordinates[1]])
                    )
                              )
            ...
            for i in range(self.distance_matrix.shape[0]):
                column.append(self.distance_func(
                    (self.distance_matrix[coordinates[0]][i], self.distance_matrix[coordinates[1]][i])
                    )
                              )
